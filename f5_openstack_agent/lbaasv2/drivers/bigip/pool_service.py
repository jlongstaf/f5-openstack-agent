...
class PoolServiceBuilder(object):
    ...
    def get_pool_status(self, service, bigip, status_keys):
        ...
        :param bigip: BIG-IP to get pool status from.
        ...
        part = pool["partition"]
        try:
            pool_status = self.pool_helper.get_stats(
                bigip,
                name=pool["name"],
                partition=part,
            ...
            pool_status = {}
            LOG.error("Error getting pool status: %s", e.message)
